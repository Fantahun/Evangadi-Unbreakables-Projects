...
const Tasks = () => {
    ...
    // Get tasks from local Storage and if not found make it []
    ...
    const editTask = (index) => {
        const newText = prompt('EditF Task:', tasks[index].text);
        if (newText) {
            const newTasks = [...tasks];
    ...
    return (
        <div>
            {tasks.length === 0 ? (
                <div>
                    <h6 className='text-center text-secondary m-4'>No Pending Tasks! You are all set!</h6>
                </div>
            ) : (
